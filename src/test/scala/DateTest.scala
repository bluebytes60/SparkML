...
class DateTest extends FunSuite {
  ...
  test("Test hashcode working correctly") {
    ...
    val VisHist1 = new ContactHis("1", DateUtil.parseSearchDate(d1))
    val VisHist2 = new ContactHis("1", DateUtil.parseSearchDate(d1))
    val set1 = HashSet(VisHist1, VisHist2)
    assert(set1.size == 1)
    val d2 = "2015-05-08 07:29:16.0"
    val VisHist3 = new ContactHis("1", DateUtil.parseSearchDate(d2))
