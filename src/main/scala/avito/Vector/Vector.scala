package avito.Vector

import java.util.Calendar

import avito.dao._
import avito.features.{PriorClicks, Feature}
...
object Vector {
  ...
  def textFeatures(seq: Seq[Any], paramFeatures: Map[String, Double], queryFeatures: Map[String, Double], titleFeatures: Map[String, Double]): Seq[Any] = {
    var v = Seq()
    val searchInfo = Feature.extract(seq, classOf[SearchInfo]).asInstanceOf[Option[SearchInfo]]
    val adsInfo = Feature.extract(seq, classOf[AdsInfo]).asInstanceOf[Option[AdsInfo]]
    ...
    if (searchInfo != None) {
      val s = searchInfo.get
      val values = s.SearchParams.filter(x => paramFeatures.contains(x)).map(x => paramFeatures(x))
      v ++ Seq(values.sum / values.size, values.min, values.max)
    } else v ++ Seq(0, 0, 0)

    if (searchInfo != None) {
      val s = searchInfo.get
      val values = s.SearchQuery.filter(x => queryFeatures.contains(x)).map(x => queryFeatures(x))
      v ++ Seq(values.sum / values.size, values.min, values.max)
    } else v ++ Seq(0, 0, 0)

    if (adsInfo != None) {
      val a = adsInfo.get
      val values = a.Title.split(" ").filter(x => titleFeatures.contains(x)).map(x => titleFeatures(x))
      v ++ Seq(values.sum / values.size, values.min, values.max)
    } else v ++ Seq(0, 0, 0)
  ...
  def CatAvgPrice(seq: Seq[Any], catAvg: Map[String, Double]): Seq[Any] = {
    var v = Seq()
    ...
    if (adsInfo != None) {
      val a = adsInfo.get
      v1 = if (catAvg.contains(a.CategoryID)) catAvg(a.CategoryID) else 0
      v ++ Seq(v1)
    } else v ++ Seq(0)
    ...
    if (searchInfo != None) {
      val s = searchInfo.get
      v2 = if (catAvg.contains(s.CategoryID)) catAvg(s.CategoryID) else 0
      v ++ Seq(v2)
    } else v ++ Seq(0)
